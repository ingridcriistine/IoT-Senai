...
arduino_port = 'COM5'  # Altere conforme sua porta serial (no Linux seria algo como /dev/ttyUSB0)
baud_rate = 115200
...
time.sleep(2)  # Esperar a conexão ser estabelecida 
    try:
        ...
        cursor = conn.cursor()
        # Sua query SQL
        cursor.execute("SELECT valor_sensor FROM bosch ORDER BY id DESC limit 1")
